...
export async function subirImagenAMeta(url) {
 ...
 */
export async function resolverMediaId(url) {
  const cacheado = await getMediaIdCache(META_PHONE_ID, url)
  if (cacheado) return cacheado

  if (enVuelo.has(url)) return enVuelo.get(url)

  const promesa = (async () => {
    const id = await subirImagenAMeta(url)
    await setMediaIdCache(META_PHONE_ID, url, id)
    return id
