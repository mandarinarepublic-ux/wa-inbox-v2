 ...
 *
 * Toda receta se crea como BORRADOR (`guardarFlujo`) y, si le tocaba quedar
 * publicada, se publica con `publicarFlujo` — el mismo camino que usa el
 * lienzo, con la misma validación y el mismo choque de Disparadores. Publicar
 * directo con `setPublicadoFlujoSupabase` (como hacía antes) se saltaba las
 * dos cosas: una receta inválida (sin anuncios y no orgánica, o con una
 * respuesta rápida ya borrada) o que choca con un flujo manual ya publicado
 * (mismo anuncio/orgánico) quedaba viva en silencio. Ahora, si `publicarFlujo`
 * devuelve `{ok:false, errores}`, el flujo se queda como borrador (ya creado,
 * visible en el lienzo para corregir a mano) y el caso entra en `sinPublicar`.
 */
export async function importarRecetas() {
  ...
  for (const item of plan) {
    ...
    const { grafo } = recetaAFlujo(item.receta, {
      sourceIds: item.sourceIds,
      organico: item.organico,
    })
    const nombre = `[receta] ${item.receta?.nombre || ''}`.trim()
    const fila = await guardarFlujo({ nombre, grafo })
    creados++
    if (item.publicado) {
      const resultado = await publicarFlujo(fila.flujo_id, true)
      if (!resultado.ok) sinPublicar.push({ nombre, errores: resultado.errores })
    }
  }

  await setAutomatizaciones({ recetas: { activo: false } })
  return { ok: true, creados, saltados, sinPublicar }
}
