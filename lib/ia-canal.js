...
export function iaActivaEnCanal(config, phoneId) {
  ...
  if (!canal) {
    // Avisar (no bloquear en pruebas/casos vacíos): si un phoneId real deja de
    // mapear a un canal conocido — le pasó al 3326 de IND el 28-jul al migrarlo
    // de cuenta — el cortafuegos deja de existir para ese número EN SILENCIO.
    // Solo con phoneId truthy: con '' o undefined (pruebas, mensajes sin
    // phone_id) no hay nada raro que avisar.
    if (phoneId) {
      console.warn('[ia-canal] phone_id sin canal conocido, cortafuegos NO aplicado:', phoneId)
    }
    return true
  }
