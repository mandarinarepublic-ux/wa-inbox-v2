...
/**
 * Mueve un adjunto UN puesto, intercambiándolo con su vecino.
 *
 * El orden es del vendedor, pero hasta ahora solo se podía fijar al cargar: para
 * corregirlo había que borrar la foto y volver a subirla. Esto lo arregla.
 *
 * ⚠️ NUNCA LANZA Y NUNCA PIERDE UN ADJUNTO. El editor la llama desde un updater
 * de React (`onChange(prev => moverAdjunto(prev, idx, -1))`), donde el índice
 * puede venir de un render anterior — por ejemplo si se mueve una foto mientras
 * otra todavía está subiendo. Ante cualquier índice o delta que no cierre,
 * devuelve la lista igual: no hacer nada se nota, perder un adjunto no.
 *
 * ⚠️ Devuelve una lista NUEVA. React compara por identidad; mutar la que recibe
 * dejaría la pantalla sin repintar.
 */
export function moverAdjunto(lista, idx, delta) {
  const base = Array.isArray(lista) ? lista.slice() : []
  if (!Number.isInteger(idx)) return base
  if (delta !== 1 && delta !== -1) return base

  const destino = idx + delta
  const dentro = (i) => i >= 0 && i < base.length
  if (!dentro(idx) || !dentro(destino)) return base

  // El objeto entero viaja tal cual: el `nombre` de un documento es lo que el
  // cliente ve en su WhatsApp, y sin él recibiría el uuid del bucket.
  ;[base[idx], base[destino]] = [base[destino], base[idx]]
  return base
}
