...
// dónde está parado.
import { avanzarDesde, evaluarCondicion, paradaDeCamino, citaDeTanda, piezasDeNodos, etapaAlPasar, deudaAlPasar } from './flujo.js'
 ...
 *          registrarPasos({telefono, flujo_id, nodoIds}), setEtapa(tel, etapa) (solo si el chat no tenía),
 *          avisar(textoHtml), ahora()→Date, cuenta?, log?, dormir?(ms) }
 * contacto = { telefono, nombre, alias, phoneId, etapa, tieneVenta, estado, ultimoEntranteAt }
...
export async function correrTanda(deps, { flujo, desde, esDisparo, contacto, wamidEntrante = '', ultimoWamid = '', respuestas = [] }) {
  const { enviar, guardarEstado, borrarEstado, registrarPasos, setEtapa, setDeuda, avisar, ahora, cuenta = '', log = console.log } = deps
  const dormir = deps.dormir || ((ms) => new Promise((listo) => setTimeout(listo, ms)))
  const ctx = {
    ultimoEntranteAt: contacto?.ultimoEntranteAt || '', etapa: contacto?.etapa || '',
    tieneVenta: !!contacto?.tieneVenta, estado: contacto?.estado || '', ahora: ahora(),
  }
  ...
  const piezas = piezasDeNodos({
    nodos: camino.mensajes, respuestas, pausasSeg: camino.pausasSeg,
  ...
  // 3) etapa del nodo ("si llegas hasta acá estás 💬 Cotizando"). `setEtapa` solo
  //    escribe si el chat no tenía etapa: el flujo nunca pisa al vendedor.
  const etapa = etapaAlPasar(camino.mensajes)
  if (etapa && setEtapa) {
    await Promise.resolve().then(() => setEtapa(contacto.telefono, etapa)).catch((e) => log('[flujo] etapa:', e.message))
  }
  // 3b) 📌 del nodo ("si llegas hasta acá, le debemos el boceto"). `setDeuda` no
  //     pisa un 📌 que ya estaba.
  const deuda = deudaAlPasar(camino.mensajes)
  if (deuda && setDeuda) {
    await Promise.resolve().then(() => setDeuda(contacto.telefono, deuda)).catch((e) => log('[flujo] 📌:', e.message))
  }
  // 4) mandar, en orden
  let salieron = 0
  for (const p of piezas) {
    // `_esperaSeg` es la pausa de la línea: se espera acá y NO viaja a /api/saliente.
    const cuerpo = { ...p }
    delete cuerpo._esperaSeg
    if (p._esperaSeg > 0) await dormir(p._esperaSeg * 1000)
