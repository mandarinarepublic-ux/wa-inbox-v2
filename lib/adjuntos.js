...
/**
 * Un documento es TODO LO DEMAS: ni foto, ni video, ni audio.
 *
 * ☠️ A proposito NO es una lista de tipos (pdf, docx, xlsx...). Es la quinta vez
 * en este repo que una lista blanca esconde algo en silencio — la ultima fueron
 * las conversaciones que desaparecian por un tipo de mensaje sin etiqueta. Con
 * una lista, el dia que alguien arrastre un .dwg o un .ai no pasaria nada en
 * pantalla y pareceria que el inbox se rompio.
 *
 * WhatsApp acepta cualquier tipo como documento, asi que el default correcto es
 * mandarlo. El tamano se controla al ENVIAR (sendDocumento), no aca.
 */
export const esDocumento = (f) => !esImagen(f?.type) && !esVideo(f?.type) && !esAudio(f)

/**
 * Extension segura sacada del nombre del archivo.
 *
 * Los documentos no tienen mapa de tipo a extension (no hay lista de tipos), asi
 * que la extension del archivo guardado sale de su nombre.
 *
 * ⚠️ Este valor termina DENTRO de la ruta del archivo en el bucket, asi que se
 * limpia a conciencia: se queda solo con letras y numeros y se corta a 8. Un
 * nombre como "x.../../../otro" no puede convertirse en un path.
 */
export function extDeNombre(nombre) {
  const limpio = String(nombre || '').trim()
  const punto = limpio.lastIndexOf('.')
  if (punto < 0 || punto === limpio.length - 1) return 'bin'
  const cruda = limpio.slice(punto + 1).toLowerCase().replace(/[^a-z0-9]/g, '')
  return cruda.slice(0, 8) || 'bin'
}
...
export function decidirAdjuntos({ actuales = 0, esVideoActual = false, entrantes = [], tope = TOPE_FOTOS } = {}) {
  ...
  // Ya no se descarta nada POR TIPO: lo que no es foto/video/audio sale como
  // documento (ver esDocumento). El unico limite es el tamano, y se mira al enviar.
  const sirven = lista
  ...
  const fotos = sirven.filter(f => esImagen(f?.type))

  // El DOCUMENTO entra ULTIMO, y solo si no vino nada visual.
  //
  // ☠️ Si ganara, arrastrar una carpeta con fotos y un readme.txt mandaria el
  // readme e ignoraria las fotos. Mandar fotos es el caso de todos los dias;
  // mandar un documento es la excepcion. La excepcion no puede pisar la regla.
  if (!fotos.length) {
    const doc = sirven.find(f => esDocumento(f))
    if (!doc) return nada
    return {
      accion: 'reemplazar', tipo: 'documento', archivos: [doc],
      aviso: sirven.length > 1 ? 'El documento va solo: se dejó ese y se ignoró el resto.' : '',
    }
  }

  const descartados = lista.length - fotos.length
  const avisoMezcla = descartados > 0
    ? `Se tomaron ${fotos.length} imagen${fotos.length > 1 ? 'es' : ''}; el resto no se mandó — un documento va solo.`
    : ''
