...
export function decidirAdjuntos({ actuales = 0, esVideoActual = false, entrantes = [], tope = TOPE_FOTOS } = {}) {
  ...
  const sirven = lista.filter(f => esImagen(f?.type) || esVideo(f?.type))
  if (!sirven.length) {
    return { ...nada, aviso: 'Eso no es una foto ni un video — por acá solo se pueden mandar imágenes y videos.' }
  }
