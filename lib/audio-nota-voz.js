...
export async function convertirANotaDeVoz(file, { onProgreso } = {}) {
  ...
  return await new Promise((resolve, reject) => {
    ...
    // De a trozos, avisando del avance: un audio largo puede tardar y el vendedor
    // necesita ver que algo pasa en vez de un botón mudo.
    const TROZO = 48000 * 5   // 5 segundos por vuelta
    for (let i = 0; i < mono.length; i += TROZO) {
      worker.postMessage({ command: 'encode', buffers: [mono.subarray(i, Math.min(i + TROZO, mono.length))] })
