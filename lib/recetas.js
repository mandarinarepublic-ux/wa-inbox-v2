...
// Tope de piezas por receta: una receta con demasiados adjuntos satura /api/saliente
// (una llamada por pieza, en serie) y puede pisar el margen de maxDuration=60 del
// webhook. Se corta DESPUÉS de armar todo, aunque eso deje afuera la pregunta final.
...
export const pieza = (contacto, extra) => ({
...
export const interactivo = (texto, botones) => ({
 ...
 */
/**
 * `citaId`: wamid del mensaje del cliente que disparó la receta. La PRIMERA pieza
 * sale citándolo — igual que cuando el vendedor toca "Responder" sobre la
 * burbuja — para que el paquete no se sienta automático. Solo la primera: si
 * citaran todas, el cliente vería su propio mensaje repetido bajo cada foto.
 * Es la misma regla de lib/cita.js para las respuestas rápidas a mano.
 */
export function piezasDeReceta({ receta, respuestas, contacto, citaId = '' }) {
  const out = []
  ...
  if (out.length > MAX_PIEZAS) {
    console.warn('[recetas] receta', receta?.id, 'con', out.length, 'piezas, se recorta a', MAX_PIEZAS)
  }
...
// Exportado: lo reusa app/api/webhook/route.js para escapar el texto de OTRAS
// alarmas de Telegram (la de "receta sin enviar"), y así no hay dos formas
// distintas de escapar HTML de Telegram conviviendo en el repo.
export const esc = (s) => String(s || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')

/**
 * Texto (HTML de Telegram) del aviso de anuncio nuevo. `tieneReceta` cambia la
 * 4ª línea: con receta ya asignada no hace falta que alguien vaya a configurarla.
 * `tipo` (`m.referral?.source_type`) distingue un anuncio de una publicación
 * orgánica boosteada ('post'): el titular de Meta dice lo mismo en los dos casos.
 */
export function textoAvisoAnuncioNuevo({ cuenta, titular, sourceId, url, tieneReceta, tipo }) {
  const rotulo = tipo === 'post' ? 'Publicación NUEVA' : 'Anuncio NUEVO'
  return [
    `📣 <b>${rotulo} en ${esc(cuenta)}</b>`,
    `«${esc(titular || '(sin titular)')}»`,
    `id <code>${esc(sourceId)}</code>`,
    tieneReceta
      ? 'Ya tiene receta asignada.'
