...
// `olvidarVersiones = false` lo usa `openConv`: abrir un chat DENTRO de la misma
// bandeja no vacía la pantalla, así que no hay nada que proteger — y olvidarlas
// obligaba a bajar la lista completa (~5.000 filas) en cada clic a un chat en vez
// de un 304 (auditoría 25-sep).
export function setCanalActivo(id, olvidarVersiones = true) {
  CANAL_ACTIVO = phoneIdDeCanal(id)
  if (!olvidarVersiones) return
...
/** Cuántas versiones recuerda esta pestaña (para las pruebas). */
export function versionesGuardadas() { return Object.keys(ETAGS).length }
/** Solo pruebas: simula que ya se vio una versión de una bandeja. */
export function _recordarVersion(clave, v) { ETAGS[clave] = v }

// ¿Esta pestaña corre un bundle distinto del que está desplegado? Un deploy NO
// llega a una pestaña ya abierta, y el bundle trae horneados los phone_id de
// lib/canales.js. Vacío en cualquiera de los dos lados (local, preview) = no se sabe.
let BUILD_SERVIDOR = ''
export function hayVersionNueva() {
  const cliente = process.env.BUILD_CLIENTE || ''
  return !!(cliente && BUILD_SERVIDOR && cliente !== BUILD_SERVIDOR)
}
export { CANALES }
// lib/api-client.js
...
export async function fetchHilo(telefono, limite = 800, canal = '', antesDe = '') {
  try {
    ...
    // `antesDe` pide el tramo ANTERIOR (subir en el chat). Vacío = los últimos.
    const antes = antesDe ? `&antesDe=${encodeURIComponent(antesDe)}` : ''
    const res = await fetch(`/api/hilo?phone=${encodeURIComponent(telefono)}&limite=${limite}&canal=${c}${antes}&t=${Date.now()}`, { cache: 'no-store' })
    if (!res.ok) throw new Error(`HTTP ${res.status}`)
    return await res.json()
  } catch (err) {
...
export async function fetchInboxSync(todosLosCanales = false) {
  try {
    ...
    const buildHdr = res.headers.get('x-build')
    if (buildHdr) BUILD_SERVIDOR = buildHdr
    ...
    if (data && data.build) BUILD_SERVIDOR = data.build
    // El canal con el que el BACKEND filtró esta respuesta, pegado a ella.
    //
    // Va acá y NO se vuelve a leer de `CANAL_ACTIVO` en `App.load()`: entre que
    // sale la petición y llega la respuesta el vendedor puede haber cambiado de
    // pestaña, y ahí las dos lecturas ya no dicen lo mismo. Con el canal pegado
    // a la respuesta, las TRES fuentes de la bandeja (`lista`, `rows` y el caché
    // de hilos) quedan filtradas por el mismo criterio y no por dos distintos.
    // Vacío = GENERAL, que muestra los dos números.
    return { ...data, canalPedido: canal === 'todos' ? '' : canal }
  } catch (err) {
...
async function patchContacto(telefono, campo, valor, phoneId = '') {
  try {
    const res = await fetch('/api/contactos/estado', {
      ...
      // CONVERSACIÓN (uno por número). Nombre, alias, etapa, 📌, notas y modo IA
...
// ── Gestión del chat (port desde IND) — todo es de la PERSONA ──────
// La temperatura ya no se marca: sale sola del último mensaje del cliente.
export async function fetchPedidosChat() {
  try {
    const r = await fetch('/api/pedidos-chat', { cache: 'no-store' })
    if (!r.ok) return null
    const d = await r.json()
    return d?.pedidos || {}
  } catch { return null }
}
export const updateEtapa = (telefono, etapa) => patchContacto(telefono, 'etapa', etapa || '')
export const updateDeuda = (telefono, nota) => patchContacto(telefono, 'deuda', nota || '')
export const updateSinAutomaticos = (telefono, on) => patchContacto(telefono, 'sinAutomaticos', Boolean(on))
export const updateTipoContacto = (telefono, tipo) => patchContacto(telefono, 'tipoContacto', tipo === 'interno' ? 'interno' : 'cliente')
...
export async function subirAudioNota(audioFile, { onProgreso } = {}) {
  ...
  if (necesitaConversion(audioFile)) {
    try {
      ...
      throw new Error(`No se pudo convertir el audio a nota de voz: ${e.message}`, { cause: e })
    }
  }
...
// FLUJOS (creador visual de nodos). Ver docs/superpowers/specs/2026-09-15-flujos-lienzo-design.md.
export async function getFlujos() {
  try {
    const res = await fetch(`/api/flujos?t=${Date.now()}`, { cache: 'no-store' })
    return await res.json()
  } catch (err) {
    console.error('[api-client] getFlujos:', err)
    return { ok: false, flujos: [] }
  }
}

// Contadores por nodo del lienzo: { ok, porNodo: { [nodoId]: clientes }, dias }.
export async function getPasosFlujo(flujo_id, dias = 30) {
  try {
    ...
    console.error('[api-client] getPasosFlujo:', err)
    return { ok: false, porNodo: {} }
  }
}

// Guardar borrador: crea si no viene flujo_id.
export async function saveFlujo({ flujo_id, nombre, grafo }) {
  try {
    const res = await fetch('/api/flujos', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ flujo_id, nombre, grafo }),
    })
    return await res.json().catch(() => ({ ok: res.ok }))
  } catch (err) {
    return { ok: false, error: err.message }
...
// publicar:true valida y publica; publicar:false despublica. Un error de
// validación vuelve como { ok:false, errores:[...] }, no como excepción.
export async function publicarFlujo(flujo_id, publicar) {
  try {
    const res = await fetch('/api/flujos/publicar', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ flujo_id, publicar }),
    })
    return await res.json().catch(() => ({ ok: res.ok }))
  } catch (err) {
    return { ok: false, error: err.message }
...
export async function deleteFlujo(flujo_id) {
  try {
    const res = await fetch(`/api/flujos?flujo_id=${encodeURIComponent(flujo_id)}`, { method: 'DELETE' })
    return await res.json().catch(() => ({ ok: res.ok }))
  } catch (err) {
    return { ok: false, error: err.message }
  }
...
// Conversión única de recetas de bienvenida a flujos (idempotente). Devuelve
// { ok, creados, saltados, sinPublicar:[{nombre, errores}] } — una receta que
// choca con otro flujo publicado o que valida mal queda como BORRADOR (creada
// igual, sin publicar) y aparece en `sinPublicar` para corregirla a mano.
export async function importarRecetas() {
  try {
    const res = await fetch('/api/flujos/importar-recetas', { method: 'POST' })
    return await res.json().catch(() => ({ ok: res.ok }))
  } catch (err) {
    return { ok: false, error: err.message }
  }
...
// Una sola puerta para el reenvío, en vez de tocar las funciones de envío que ya
// usa el chat (esas son la ruta caliente y tienen cada una su historia).
//
// ☠️ `canal` es OBLIGATORIO y es el del chat DESTINO, no el de la pestaña ni el
// del chat de origen: reenviar es escribirle a OTRA conversación, y el canal sale
// siempre de la conversación a la que se escribe.
export async function reenviarPieza(telefono, nombre, pieza, canal) {
  const base = { Telefono: telefono, Nombre: nombre || '', Canal: canal }
  const p = pieza || {}
  if (p.clase === 'imagen') {
    return postSaliente({ ...base, ImagenURL: p.url, ...(p.texto ? { Caption: p.texto } : {}) })
  }
  if (p.clase === 'video') {
    return postSaliente({ ...base, VideoURL: p.url, ...(p.texto ? { Caption: p.texto } : {}) })
  }
  if (p.clase === 'audio') {
    return postSaliente({ ...base, AudioURL: p.url })
  }
  if (p.clase === 'documento') {
    return postSaliente({ ...base, DocURL: p.url, DocNombre: p.nombre || 'documento' })
  }
  return postSaliente({ ...base, Mensaje: p.texto || '' })
}
