...
export async function fetchHilo(telefono, limite = 800) {
  try {
    const res = await fetch(`/api/hilo?phone=${encodeURIComponent(telefono)}&limite=${limite}&canal=${CANAL_ACTIVO}&t=${Date.now()}`, { cache: 'no-store' })
    if (!res.ok) throw new Error(`HTTP ${res.status}`)
    return await res.json()
  } catch (err) {
...
export async function fetchInboxSync() {
  try {
    const res = await fetch(`/api/inbox-sync?canal=${CANAL_ACTIVO}`)
    if (!res.ok) throw new Error(`HTTP ${res.status}`)
...
export async function sendReply(telefono, nombre, mensaje, contextoId = '') {
  return postSaliente({
...
export async function sendImageUrl(telefono, nombre, imageUrl, mediaId = '') {
  return postSaliente({
