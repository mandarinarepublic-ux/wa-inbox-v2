...
// Catálogo Shopify (pestaña TIENDA). Devuelve [{id,title,price,image,variants[]}].
// Sin `q` trae todo el catálogo de la tienda; el buscador filtra en el cliente.
export async function fetchProductos(q = '') {
  try {
    const res = await fetch(`/api/tienda${q ? `?q=${encodeURIComponent(q)}` : ''}`, { cache: 'no-store' })
    if (!res.ok) throw new Error(`HTTP ${res.status}`)
