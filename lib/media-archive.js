...
//
// Al recibir un medio (foto, audio, video o documento), Meta solo nos da un
// `media_id` temporal (caduca ~30 días y exige el token para bajarse). Este
// módulo baja el binario UNA vez y lo guarda en nuestro bucket público
// `inbox-media`, dejando una URL ESTABLE en inbox.mensajes.media_url. Desde ahí
// el CRM y el inbox la leen directo con un <img src>/<audio src>, sin depender
// de Meta y sin que el medio caduque nunca.
//
...
/**
 * Archiva el medio de un mensaje entrante y devuelve la URL pública (o null).
 ...
 */
export async function archivarMedia({ mediaId, wamid, cuenta = CUENTA }) {
  try {
    ...
    const contentType = (bin.headers.get('content-type') || meta.mime_type || 'application/octet-stream').split(';')[0].trim()
