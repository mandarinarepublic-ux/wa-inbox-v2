...
import { esBsuid } from './cliente-sin-telefono.js'
...
// Google Sheets quedó FUERA: Supabase es la única fuente (lectura y escritura).
// Se fuerza 'supabase' a propósito (se ignora el env DATA_BACKEND) para que el
// Sheet no se consulte ni se escriba nunca más.
...
export function getSupabase() {
  ...
  _client = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, {
    ...
    // CRÍTICO: Next.js (App Router) parcha el fetch global y cachea las respuestas GET
    // de PostgREST → las lecturas quedaban congeladas en el conteo de la primera llamada
    // (nunca veían filas nuevas). Forzamos no-store para que cada lectura sea en vivo.
...
export function canonTel(s) {
  // Un BSUID (cliente con nombre de usuario, sin número) se guarda TAL CUAL: sacarle
  // los dígitos inventaba un "teléfono" al que Meta jamás podría entregar.
  if (esBsuid(s)) return String(s).trim()
 ...
 */
// Espejo a Google Sheets APAGADO: solo se escribe a Supabase (el arg sheetsFn se
// mantiene por compatibilidad de firma pero NO se ejecuta).
export async function dualWrite(sheetsFn, supabaseFn) {
  return supabaseFn()
}

/** Lectura: SOLO Supabase. Google Sheets no se consulta. */
