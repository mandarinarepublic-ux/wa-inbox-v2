...
import { mapaBandeja, patchesDeMensaje } from './bandeja.js'
...
/**
 * Filas de la bandeja. `canal = null` trae las de TODOS los números (GENERAL).
 *
 * Se pagina igual que `getContactosSupabase`: PostgREST corta en 1000 filas por
 * request e **ignora un `.limit()` mayor sin avisar**. Este archivo ya se comió ese
 * rastrillo dos veces (la última dejó 1.391 conversaciones invisibles), y la
 * bandeja arranca con 1.642 filas en MANDI — o sea que ya nace del otro lado del
 * tope. El orden es por la clave primaria completa: es un orden TOTAL, y sin eso
 * dos páginas pueden repetir una fila y saltarse otra.
 */
export async function getBandejaSupabase(canal = canalPorDefecto()) {
  const sb = getSupabase()
  const pageSize = 1000
  let from = 0
  const filas = []
  for (;;) {
    let q = sb.from('bandeja')
      .select('telefono, phone_id, estado, no_leidos, ultimo_mensaje_at, ultimo_entrante_at, ultimo_push_at, alerta_ventana_at')
      .eq('cuenta', CUENTA)
    if (canal) q = q.eq('phone_id', canal)   // canal null → todos los números
    const { data, error } = await q
      .order('telefono', { ascending: true })
      .order('phone_id', { ascending: true })  // orden TOTAL: la clave sin el conflicto
      .range(from, from + pageSize - 1)
    if (error) throw error
    const lote = data || []
    filas.push(...lote)
...
/** Igual que `getBandejaSupabase` pero ya indexado por (teléfono, canal). */
export async function getMapaBandejaSupabase(canal = canalPorDefecto()) {
  return mapaBandeja(await getBandejaSupabase(canal))
}
...
export async function getListaSupabase(limite = 4000, canal = canalPorDefecto()) {
  ...
  const filas = await paginarLimite(async (from, to) => {
    ...
    //
    // ⚠️ LA VISTA NO SE TOCA: el inbox de IND la lee también, y todavía no está
    // migrado. Acá solo cambia CUÁL leemos, nunca su definición.
    let q = sb
      .from('ultimos_mensajes_canal')
    ...
    const { data, error } = await q
      ...
      .order('telefono', { ascending: true }) // desempate estable, ver comentario arriba
      .range(from, to)
    if (error) throw error
    return data
  }, limite)
...
export async function contarPendientesPorCanalSupabase() {
  // Se cuenta desde `bandeja` y NO desde la vista `pendientes_por_canal`.
  //
  // Dos motivos, y el segundo es una restricción dura:
  //  1. La vista cruza `conversaciones.estado` (uno por PERSONA) con el canal del
  //     mensaje, así que quien estaba pendiente contaba en los dos números aunque
  //     solo debiera uno. `bandeja` ya tiene el estado por canal: se cuenta directo.
  //  2. ⚠️ ESA VISTA LA LEE EL INBOX DE IND, que comparte esta base y todavía no
  //     está migrado. Cambiar su definición lo rompería. Se queda intacta.
  const filas = await getBandejaSupabase(null)   // null = todos los números
  const out = {}
  for (const b of filas) {
    if (String(b.estado || '').trim().toUpperCase() !== 'PENDIENTE') continue
    const k = b.phone_id || 'sin-canal'
    out[k] = (out[k] || 0) + 1
  }
  return out
}
