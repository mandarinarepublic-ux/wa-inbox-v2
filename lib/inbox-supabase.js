...
// Columnas que necesita la UI. Antes evitaba `raw` (el jsonb del webhook) por
// peso, pero sin él toMensaje no puede reconstruir el detalle de un order/
// unsupported viejo al leerlo (ver contenidoTipoEspecial) — se agrega a propósito
// acá (hilo completo y búsqueda). getListaSupabase NO puede sumarlo: sale de una
// VISTA (ultimos_mensajes/_canal) que no lo expone, y cambiar esa vista es tocar
// la base — así que la lista lateral se queda con la etiqueta genérica para las
// filas viejas (toMensaje ya sabe hacerlo sin `raw`).
const COLS_MSG = 'wa_message_id, telefono, nombre, tipo, texto, media_url, fecha, direccion, media_id, respuesta_ia, foto_ia, contexto_id, botones, referral, estado_entrega, raw'

// Descarta lo que NO TIENE NADA para pintar (ni texto, ni media, ni botones) →
// evita chats fantasma. Es la TERCERA vez que este filtro se come mensajes de
// verdad (antes: fotos sin caption, notas de voz sin mediaId). Esta vez eran
// 'order' (pedido del catálogo) y 'unsupported'/'system' — pero esos YA NO caen
// acá: lib/wa-mensaje.js les arma un `contenido` no vacío en el origen, así que
// pasan por el camino normal de "hay mensaje" sin que este filtro tenga que
// conocerlos. Lo que sigue cayendo (y con razón) es cualquier tipo —conocido o
// no— cuya fila llegó realmente vacía: eso sí es un fantasma, no una persona.
