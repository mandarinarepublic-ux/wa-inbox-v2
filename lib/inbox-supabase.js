...
export async function getListaSupabase(limite = 4000, canal = canalPorDefecto()) {
  ...
  const filas = await paginarLimite(async (from, to) => {
    ...
    let q = sb
      ...
      .select('wa_message_id, telefono, nombre, tipo, texto, media_url, fecha, direccion, media_id, botones, estado_entrega, contexto_id, referral, phone_id, estado_bandeja, no_leidos_bandeja, ultimo_entrante_canal')
      .eq('cuenta', CUENTA)
    if (canal) q = q.eq('phone_id', canal)   // canal null → todos los números (pestaña GENERAL)
    const { data, error } = await q
