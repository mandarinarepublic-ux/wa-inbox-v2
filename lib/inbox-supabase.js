...
export async function getContactosSupabase() {
  ...
  for (;;) {
    const { data, error } = await sb
      .from('conversaciones').select('*').eq('cuenta', CUENTA)
 ...
 */
export async function getHiloSupabase(telefono, limite = 800) {
  const t9 = soloDigitos(telefono).slice(-9)
 ...
 * (vista inbox.ultimos_mensajes). Hace que aparezcan también los chats viejos que
 * quedaban fuera de la ventana de getMensajesSupabase(3000) — el bug de "no aparece
 * el cliente / se borraron los mensajes".
 */
export async function getListaSupabase(limite = 4000) {
  const sb = getSupabase()
  const { data, error } = await sb
    ...
    .select('wa_message_id, telefono, nombre, tipo, texto, media_url, fecha, direccion, media_id, botones, estado_entrega, contexto_id, referral')
    .eq('cuenta', CUENTA)
    .order('fecha', { ascending: false }).limit(limite)
  if (error) throw error
