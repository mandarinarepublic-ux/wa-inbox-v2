...
import { contenidoTipoEspecial, parseUbicacion } from './wa-mensaje.js'
import { reparacionesDeReentrega } from './reentrega.js'
import { pedidoDeMensaje } from './catalogo.js'
import { notaParaNumeroNuevo } from './cambio-numero.js'
import { patchesDeMensaje, reabrePorEntregaFallida } from './bandeja.js'
import { guardarAdjuntos } from './adjuntos-respuesta.js'
import { esEtapa } from './gestion.js'
import { acuseAgrupado } from './version-inbox.js'
import { detectarPromesa, cumplePromesaAlToque } from './promesas.js'
...
/**
 * Las columnas que se le piden a `conversaciones` para armar la ficha del chat.
 *
 * ⚠️ ANTES ERA `select('*')`, y eso costaba caro: traía las 30 columnas de las
 * 1.804 conversaciones para usar 16. Medido el 30-ago: **1.585 kB → 782 kB**, y
 * esta pieza era el **65% del payload** de `/api/inbox-sync`, que a su vez es
 * casi todo el Fast Origin Transfer — el rubro más caro de la factura.
 *
 * ☠️ EL RIESGO QUE ABRE, y por qué hay una prueba. Si alguien agrega un campo a
 * `toContacto` y olvida la columna acá, ese campo llega `undefined` PARA
 * SIEMPRE. No revienta nada: simplemente el alias no se ve, o la temperatura no
 * se guarda, o la ventana de 24 h se pinta mal. Familia "la pantalla miente".
 * `tests/columnas-contacto.test.js` LEE el código de `toContacto` y se cae si
 * toca una columna que no está pedida — y también si sobra alguna.
 *
 * ⚠️ NO agregar aquí un filtro por canal. La ficha es UNA por persona, no una
 * por número: filtrarla dejaba al cliente que escribió a los dos números con su
 * lista en un canal y su estado en el otro, y ARCHIVAR parecía no funcionar.
 * Ver el comentario de `/api/inbox-sync`.
 */
export const COLS_CONTACTO = [
  'telefono', 'nombre_contacto', 'alias', 'estado', 'wa_id', 'phone_id',
  'modo_ia', 'id_venta', 'notas', 'temperatura',
  'etapa', 'etapa_por', 'etapa_at', 'deuda_nota', 'deuda_por', 'deuda_at',
  'sin_automaticos', 'tipo_contacto', 'reactivacion_n', 'reactivacion_at', 'ultimo_humano_at',
  'ultimo_mensaje_at', 'ultimo_entrante_at', 'ultimo_seguimiento_at',
  'alerta_ventana_at', 'ultimo_push_at', 'ultimo_aviso_telegram_at',
  'ultima_receta_at', 'username',
].join(', ')
...
function toContacto(c) {
  return {
    ...
    waId:     c.wa_id || '',
    username: c.username || '',   // @usuario (clientes que escriben sin número)
    ...
    // Gestión (port desde IND, diseño 2026-09-22). La pantalla de MANDI todavía no
    // las muestra (etapa 2 del port); los flujos, las promesas y la IA ya las escriben.
    etapa:          esEtapa(c.etapa) ? c.etapa : '',
    etapaPor:       c.etapa_por || '',
    etapaAt:        c.etapa_at || null,
    deudaNota:      c.deuda_nota || '',
    deudaPor:       c.deuda_por || '',
    deudaAt:        c.deuda_at || null,
    sinAutomaticos: c.sin_automaticos === true,
    tipoContacto:   c.tipo_contacto === 'interno' ? 'interno' : 'cliente',
    reactivacionN:  Number(c.reactivacion_n) || 0,
    reactivacionAt: c.reactivacion_at || null,
    ultimoHumanoAt: c.ultimo_humano_at || null,
  }
}
...
export async function getContactosSupabase(canal = canalPorDefecto()) {
  ...
  const filas = []
  for (;;) {
    let q = sb.from('conversaciones').select(COLS_CONTACTO).eq('cuenta', CUENTA)
...
export async function registrarContactoEntranteSupabase(telefono, nombre, waId, username = '') {
  const sb = getSupabase()
  const tel = canonTel(telefono) || String(telefono)
  const { data: exist } = await sb
    .from('conversaciones').select('conversacion_id, nombre_contacto, wa_id, estado, reactivacion_n, username')
  ...
  if (!exist) {
    const { error } = await sb.from('conversaciones').insert({
      ...
      username: username || null,   // @usuario de WhatsApp (clientes sin número)
  ...
  // El @usuario SÍ se actualiza: la persona puede cambiarlo y es lo único que se ve
  // de ella cuando no dio número.
  if (username && username !== exist.username) patch.username = username
  ...
  // El cliente habló: la reactivación vuelve a empezar de cero (port desde IND).
  if (Number(exist.reactivacion_n) > 0) patch.reactivacion_n = 0
...
/**
 * Pone el nombre de la agenda del celular SOLO si la conversación existe y no
 * tiene nombre. No crea conversaciones: la agenda del teléfono trae cientos de
 * contactos que nunca escribieron, y cada fila nueva sería un chat fantasma en
 * la bandeja. Un nombre ya escrito (por Meta o a mano) tampoco se pisa.
 */
export async function ponerNombreSiFaltaSupabase(telefono, nombre) {
  const sb = getSupabase()
  const tel = canonTel(telefono) || String(telefono)
  const limpio = String(nombre || '').trim()
  if (!tel || !limpio) return { ok: false, motivo: 'sin datos' }
  const { data: exist } = await sb
    .from('conversaciones').select('conversacion_id, nombre_contacto')
    .eq('cuenta', CUENTA).eq('telefono', tel).maybeSingle()
  if (!exist) return { ok: true, tocado: false }
  if (String(exist.nombre_contacto || '').trim()) return { ok: true, tocado: false }
  const { error } = await sb.from('conversaciones')
    .update({ nombre_contacto: limpio }).eq('conversacion_id', exist.conversacion_id)
  if (error) throw error
  return { ok: true, tocado: true }
}
...
// ─── Gestión (port desde IND, diseño 2026-09-22) ─────────────────────────────
// Varios campos en UNA escritura; `ajustar` agrega condiciones al UPDATE para que
// la regla ("solo si no había 📌", "solo si no había etapa") viva en la base.
async function setCamposContacto(telefono, patch, ajustar = (q) => q) {
  const sb = getSupabase()
  const convId = await getConvId(String(telefono))
  const { error } = await ajustar(sb.from('conversaciones').update(patch).eq('conversacion_id', convId))
  if (error) throw error
  return { ok: true }
}

/** Etapa de la venta. '' la borra. `soloSiVacia`: el flujo nunca pisa al vendedor. */
export const updateEtapaSupabase = (tel, etapa, por = 'humano', { soloSiVacia = false } = {}) => {
  if (!esEtapa(etapa)) return setCamposContacto(tel, { etapa: null, etapa_por: null, etapa_at: null })
  return setCamposContacto(
    tel,
    { etapa, etapa_por: por === 'flujo' ? 'flujo' : 'humano', etapa_at: new Date().toISOString() },
    // `soloSiVacia` (flujos): nunca pisa lo que marcó una PERSONA y nunca retrocede.
    // 💬 solo entra si no hay etapa; 💳 también puede reemplazar un 💬 que puso OTRO
    // flujo (el del anuncio → el de pago). 23-sep-2026.
    soloSiVacia
      ? (q) => (etapa === 'esperando_pago'
        ? q.or('etapa.is.null,and(etapa_por.eq.flujo,etapa.eq.cotizando)')
        : q.is('etapa', null))
      : undefined,
  )
}

/** 📌 Le debemos algo. Nota vacía lo apaga. `noPisar`: conserva un 📌 que ya estaba. */
export const updateDeudaSupabase = (tel, nota, por = 'humano', { noPisar = false } = {}) => {
  const texto = String(nota ?? '').trim()
  if (!texto) return setCamposContacto(tel, { deuda_nota: null, deuda_por: null, deuda_at: null })
  const quien = ['humano', 'auto', 'ia'].includes(por) ? por : 'humano'
  return setCamposContacto(
    tel,
    { deuda_nota: texto.slice(0, 120), deuda_por: quien, deuda_at: new Date().toISOString() },
    noPisar ? (q) => q.is('deuda_at', null) : undefined,
  )
}

export const updateSinAutomaticosSupabase = (tel, on) => setCamposContacto(tel, { sin_automaticos: Boolean(on) })
export const updateTipoContactoSupabase = (tel, tipo) =>
  setCamposContacto(tel, { tipo_contacto: tipo === 'interno' ? 'interno' : 'cliente' })

/**
 * 📌 🤖 Después de que una PERSONA manda algo: si el texto promete algo, se prende
 * 📌 automático (sin pisar uno que ya estaba); si es foto/video/documento y había
 * un 📌 automático de hace ≤15 min, se apaga (cumplió al toque). lib/promesas.js
 */
export async function revisarPromesaSupabase(tel, { tipo, texto } = {}) {
  const t = canonTel(tel) || String(tel)
  if (tipo === 'texto') {
    const p = detectarPromesa(texto)
    return p ? updateDeudaSupabase(t, p.frase, 'auto', { noPisar: true }) : { ok: true }
  }
  const sb = getSupabase()
  const { data } = await sb.from('conversaciones').select('deuda_por, deuda_at')
    .eq('cuenta', CUENTA).eq('telefono', t).maybeSingle()
  if (cumplePromesaAlToque({ deudaPor: data?.deuda_por, deudaAt: data?.deuda_at, tipoEnviado: tipo })) {
    return setCamposContacto(t, { deuda_nota: null, deuda_por: null, deuda_at: null },
      (q) => q.eq('deuda_por', 'auto').eq('deuda_at', data.deuda_at))
  }
  return { ok: true }
}

/**
 * Escribió una PERSONA (/api/saliente sin `auto`): los automáticos no cuentan como atención.
 * ⚠️ NO reinicia el contador de toques de la reactivación: si lo hiciera, el cliente
 * recibiría el mismo toque dos veces en la misma ventana. Solo el CLIENTE lo reinicia
 * al escribir (registrarContactoEntranteSupabase).
 */
export const marcarRespuestaHumanaSupabase = (tel) =>
  setCamposContacto(tel, { ultimo_humano_at: new Date().toISOString() })

/**
 * Estado de bandeja de UNA conversación (teléfono + número), leído en el momento.
 * Lo usa la reactivación justo antes de reservar: en MANDI el estado que manda es
 * el del número, no el de `conversaciones` (que es el del último cambio de
 * CUALQUIERA de los dos números). null = no hay fila.
 */
export async function getEstadoBandejaSupabase(tel, phoneId) {
  if (!phoneId) return null
  const sb = getSupabase()
  const t = canonTel(tel) || String(tel)
  const { data, error } = await sb.from('bandeja').select('estado')
    .eq('cuenta', CUENTA).eq('telefono', t).eq('phone_id', String(phoneId)).maybeSingle()
  if (error) throw error
  return data ? String(data.estado || '').toUpperCase() : null
}

/**
 * RESERVA el toque de reactivación ANTES de enviarlo (port desde IND, revisión I1
 * e I-B). Un solo UPDATE condicional: solo gana si el contador sigue en
 * `nEsperado`, nadie puso 📌 ni 🤫, y ni el cliente ni una persona escribieron
 * desde la foto que se usó para decidir. Si otra corrida ya lo tomó o algo cambió,
 * devuelve false y NO se envía nada. Si después el envío falla, el toque queda
 * saltado: mejor uno de menos que uno repetido.
 *
 * La bandeja del NÚMERO se revisa aparte (getEstadoBandejaSupabase), justo antes:
 * no está en esta tabla. `conversaciones.estado` = 'ATENDIDO' se exige igual como
 * freno extra (si alguien reabrió cualquiera de los dos números, no se escribe).
 */
export async function reclamarReactivacionSupabase(tel, { nEsperado, nNuevo, ultimoEntranteAt, ultimoHumanoAt }) {
  const sb = getSupabase()
  const t = canonTel(tel) || String(tel)
  const ahora = new Date().toISOString()
  let q = sb.from('conversaciones')
    .update({ reactivacion_n: nNuevo, reactivacion_at: ahora, ultimo_seguimiento_at: ahora })
    .eq('cuenta', CUENTA).eq('telefono', t).eq('reactivacion_n', Number(nEsperado) || 0)
    .eq('estado', 'ATENDIDO').is('deuda_at', null).eq('sin_automaticos', false)
  q = ultimoEntranteAt ? q.eq('ultimo_entrante_at', ultimoEntranteAt) : q.is('ultimo_entrante_at', null)
  q = ultimoHumanoAt ? q.eq('ultimo_humano_at', ultimoHumanoAt) : q.is('ultimo_humano_at', null)
  const { data, error } = await q.select('conversacion_id')
  if (error) throw error
  return Array.isArray(data) && data.length === 1
}

/**
 * Etiqueta CRM: último pedido no cancelado por últimos 9 dígitos (vista
 * inbox.pedido_por_telefono, cualquier tienda). Paginado: PostgREST corta en 1000.
 */
export async function getPedidosPorTelefonoSupabase() {
  const sb = getSupabase()
  const desde = new Date(Date.now() - 60 * 24 * 3600 * 1000).toISOString()
  const out = {}
  for (let from = 0; ; from += 1000) {
    const { data, error } = await sb.from('pedido_por_telefono')
      .select('t9, pedido_id, estado_pedido, estado_pago, monto_pendiente, fecha_pedido, fecha_actualizacion')
      .or(`fecha_pedido.gte.${desde},estado_pedido.in.(EN_FABRICA,DESPACHO),and(estado_pago.eq.ABONO,monto_pendiente.gt.0)`)
      .order('t9', { ascending: true })
      .range(from, from + 999)
    if (error) throw error
    for (const r of data || []) out[r.t9] = r
...
// Devuelve además `avisado`: si la fila YA existía, dice si ya se mandó el aviso de
// Telegram por ESE anuncio (`avisado_at`), para que el webhook decida con la
// compuerta correcta (avisado_at is null), no con "la fila se creó ahora".
export async function registrarAnuncioVistoSupabase({ sourceId, referral }) {
  const sb = getSupabase()
  const sid = String(sourceId || '').trim()
  if (!sid) return { ok: false, nuevo: false, avisado: false }
  const { data: exist, error: errExist } = await sb.from('anuncios').select('source_id, visto_en, etiqueta, avisado_at')
  ...
  if (!exist) {
    ...
    if (error && !/duplicate key/i.test(error.message)) throw error
    return { ok: true, nuevo: !error, avisado: false }
  }
  if (exist.visto_en) return { ok: true, nuevo: false, avisado: Boolean(exist.avisado_at) }
  // Guardia contra la reentrega de Meta (~0,4 s después, ver lib/reentrega.js): si
  // dos procesos llegan acá a la vez, el WHERE visto_en IS NULL solo lo gana uno;
  // el que pierde tiene que verlo en las filas tocadas, no asumir éxito.
  const { data, error } = await sb.from('anuncios').update({ ...snap, visto_en: new Date().toISOString() })
    .eq('cuenta', CUENTA).eq('source_id', sid).is('visto_en', null)
    .select('source_id')
  if (error) throw error
  return { ok: true, nuevo: (data || []).length > 0, avisado: Boolean(exist.avisado_at) }
}

// Reclama el aviso ANTES de mandarlo a Telegram (mismo patrón que
// marcarRecetaSupabase): con dos invocaciones de `procesar` corriendo a la vez
// para el mismo anuncio nuevo (dos POST del webhook, cada uno con su propio
// waitUntil), las dos leen `avisado_at IS NULL` en el SELECT de
// registrarAnuncioVistoSupabase y las dos mandarían el Telegram si la guardia
// fuera solo un UPDATE después del envío. El WHERE acá es el candado real: el
// UPDATE con guardia solo lo gana UN proceso, y el `data` devuelto (no un
// "no hubo error") es la única prueba de quién ganó.
export async function reclamarAvisoAnuncioSupabase(sourceId) {
  const sb = getSupabase()
  const { data, error } = await sb.from('anuncios').update({ avisado_at: new Date().toISOString() })
    .eq('cuenta', CUENTA).eq('source_id', String(sourceId)).is('avisado_at', null)
    .select('source_id')
  if (error) throw error
  return { ok: true, reclamado: (data || []).length > 0 }
}

// Devuelve el reclamo cuando el envío a Telegram falló: sin esto, un anuncio
// nuevo que justo coincidió con Telegram caído queda con `avisado_at` puesto
// pero SIN que nadie se haya enterado — el próximo referral del mismo anuncio
// ya no reintentaría nunca.
export async function liberarAvisoAnuncioSupabase(sourceId) {
  const sb = getSupabase()
  const { error } = await sb.from('anuncios').update({ avisado_at: null })
...
export function toMensaje(m) {
  ...
  return {
    ...
    // Ubicacion de Google Maps compartida por WhatsApp. Llega guardada como
    // texto ("PIN lat,lon nombre") y se derivaba en coordenadas pelonas en el
    // chat; aca se arma el objeto {lat, lon, nombre, direccion, url} que la
    // burbuja pinta como tarjeta clicable. `raw` NO se manda al cliente (medio
    // mega de jsonb por poll), solo este objetito. null cuando no aplica.
    ubicacion:      parseUbicacion(m.texto, m.raw),
    ...
    // De qué ANUNCIO vino esta CONVERSACIÓN (no este mensaje).
    //
    // ⚠️ Es distinto de `referral`, que es del último mensaje. Cuando un chat
    // espera respuesta, el último suele ser un seguimiento ("y en talla M?") y el
    // referral quedó en el PRIMER mensaje: medido el 4-sep, el referral del último
    // se dispara en 0 de 20 chats pendientes de MANDI. Sale de
    // `conversaciones.ctwa_source_id` (ya se guardaba) traducido por
    // `inbox.anuncios`. Solo viene de `lista_bandeja`; en las demás lecturas, ''.
    origenAnuncio:  m.origen_anuncio ? String(m.origen_anuncio).trim() : '',
    // Cuando el cliente se mudo de telefono, la nota que se le deja al numero
    // NUEVO guarda de cual venia. Se expone para pintar el boton que lleva al
    // chat viejo, sin que la burbuja tenga que adivinarlo del texto.
    numeroAnterior: m.raw?.system?.numero_anterior || '',
...
export async function actualizarEstadoEntregaSupabase(wamid, nuevo) {
  ...
  // `.select()` para saber A QUIÉN y POR QUÉ CANAL era el mensaje sin un segundo
  // viaje, y de paso cuántas filas tocó: cero no es error, es un wamid que no
  // conocemos (o un status repetido que ya se aplicó).
  const { data, error } = await q.select('telefono, phone_id')
  if (error) throw error

  // ── La red de seguridad ────────────────────────────────────────────────────
  // Meta contesta 200 con wamid al enviar, así que el chat ya pasó a ATENDIDO y
  // salió de la bandeja. Este webhook es el ÚNICO momento en que se sabe que el
  // cliente NO recibió nada. Un mensaje que no llegó no es una respuesta: vuelve
  // a Pendientes. En IND, sin esto, 26 clientes quedaron en ATENDIDO sin haber
  // recibido un solo mensaje y nadie lo notó en 5 días.
  const fila = (data || [])[0]

  // ── Que los ✓✓ no se queden congelados ──────────────────────────────────
  // El ETag de /api/inbox-sync sale de `bandeja.actualizado_en` + el de
  // `conversaciones`. Un acuse de entrega no tocaba ninguno, así que la pantalla
  // podía mostrar "enviado" con el mensaje ya leído — "la pantalla miente" otra
  // vez, ahora por la puerta del ahorro.
  //
  // UPDATE y no upsert: si la fila no existiera, un upsert la CREARÍA con estado
  // PENDIENTE y metería un chat en la bandeja sin que nadie escribiera nada.
  // Y solo si el update de `mensajes` tocó una fila de verdad: un status repetido
  // no tiene por qué invalidar el caché de todas las pantallas.
  //
  // ☠️ 23-sep-2026: toca `acuse_en`, NO `actualizado_en`. Tocando
  // `actualizado_en`, cada acuse obligaba a todas las pantallas a recargar el
  // inbox entero: una ráfaga de fotos en IND dio 45 por minuto y tumbó la base
  // (la misma que usa este inbox). La versión cuenta `acuse_en` agrupado en
  // ventanas de 30 s (`acuseAgrupado`, lib/version-inbox.js): el ✓✓ se sigue
  // viendo, a lo más 30 s tarde.
  if (fila && fila.phone_id) {
    const telTocar = canonTel(fila.telefono) || String(fila.telefono)
    const { error: e0 } = await sb.from('bandeja')
      .update({ acuse_en: new Date().toISOString() })
      .eq('cuenta', CUENTA).eq('telefono', telTocar).eq('phone_id', String(fila.phone_id))
    if (e0) throw e0
  }

  if (fila && reabrePorEntregaFallida(nuevo, null)) {
    const tel = canonTel(fila.telefono) || String(fila.telefono)
    // La bandeja DE ESE CANAL es lo que lee esta app. Va por `upsertBandeja` para
    // que también sirva si la fila todavía no existiera.
    if (fila.phone_id) await upsertBandeja(tel, fila.phone_id, { estado: 'PENDIENTE' })
    // Y la ficha de la persona, que es lo que lee el CRM para el chat del pedido
    // (y el inbox de IND). El `.neq` hace la guarda DENTRO del mismo UPDATE: sin
    // leer antes y sin carrera entre dos statuses que llegan juntos.
    const { error: e2 } = await sb.from('conversaciones')
      .update({ estado: 'PENDIENTE' })
      .eq('cuenta', CUENTA).eq('telefono', tel).neq('estado', 'PENDIENTE')
    if (e2) throw e2
  }
  return { ok: true }
}
 ...
 *
 * ⚠️ 1000 y no 3000: PostgREST corta en 1000 sin avisar, así que la "ventana de
 * 3000" siempre fue de 1000 (auditoría 25-sep: ~44 h). Se deja explícito en vez
 * de paginar porque es la ruta CALIENTE (/api/inbox-sync): triplicarla cuesta en
 * cada ciclo, y las conversaciones fuera de la ventana igual salen por la lista.
 */
export async function getMensajesSupabase(limite = 1000, canal = canalPorDefecto()) {
 ...
 */
export async function getHiloSupabase(telefono, limite = 800, canal = canalPorDefecto(), antesDe = '') {
  limite = Math.min(limite, 1000)   // tope de PostgREST: pedir más devolvía 1000 callado
  const t9 = soloDigitos(telefono).slice(-9)
  ...
  let q = sb
    ...
    .like('telefono', `%${t9}`)
  // ⚠️ `lte` y NO `lt`, a propósito. Dos mensajes pueden compartir el mismo
  // segundo (WhatsApp da la hora al segundo y la gente manda varios seguidos).
  // Con `lt`, los que empatan en la fecha del borde se perderían PARA SIEMPRE y
  // sin que nadie se entere. Con `lte` algunos vuelven repetidos, y eso se
  // deduplica en el navegador (`fusionarHilo`). Un duplicado es invisible; un
  // hueco es un mensaje de un cliente que desaparece.
  if (antesDe) q = q.lte('fecha', antesDe)
  const { data, error } = await q
    .order('fecha', { ascending: false }).limit(limite)
  if (error) throw error
  const msgs = (data || []).reverse().map(toMensaje).filter(esPintable)
  return await conPedidosResueltos(msgs, data || [])
}

/**
 * Le pone nombre y foto a los pedidos del catalogo de WhatsApp.
 *
 * Meta manda SOLO el `product_retailer_id` en un pedido: el chat mostraba
 * "1 x $35.00 (44500256129117)" y nadie sabia que se habia vendido. Aca se
 * resuelve contra el catalogo del CRM y, si no esta, contra Meta.
 * El porque y los numeros, en lib/catalogo.js.
 *
 * Solo corre si el hilo TIENE un pedido: son raros (20 en dos meses entre las
 * dos cuentas), asi que abrir un chat normal no paga nada.
 */
async function conPedidosResueltos(msgs, filas) {
  const crudoPorId = new Map(filas.map((f) => [f.wa_message_id, f.raw]))
  const conPedido = msgs.filter((m) => m.tipo === 'order' && crudoPorId.get(m.id)?.order)
  if (!conPedido.length) return msgs
  await Promise.all(conPedido.map(async (m) => {
    // Nunca tumba el hilo: si la resolucion falla, el mensaje se queda como
    // estaba (con su id a la vista), que es lo honesto.
    m.pedido = await pedidoDeMensaje(crudoPorId.get(m.id)).catch(() => null)
  }))
  return msgs
}
...
/**
 * Las dos marcas de tiempo que definen "el estado de este inbox". Alimentan el
 * ETag de /api/inbox-sync (ver lib/version-inbox.js).
 *
 * Son dos `max()` sobre índices dedicados: medido con EXPLAIN ANALYZE, 5,4 ms
 * contra los ~8 viajes del ciclo completo. Esa es toda la gracia — preguntar
 * "¿cambió algo?" tiene que costar mucho menos que traer la respuesta.
 *
 * ⚠️ Devuelve `[null, null]` si algo falla, NUNCA lanza: la versión es una
 * optimización, y quedarse sin ella significa mandar todo (que es lo correcto),
 * no romper el inbox.
 */
export async function versionInboxSupabase() {
  const sb = getSupabase()
  try {
    const [b, c, a] = await Promise.all([
      // ☠️ SIN filtrar por canal, a propósito. El payload lleva `pendientes` de
      // TODOS los números —es lo que alimenta el contador del botón de la otra
      // bandeja—, así que un mensaje que entra por el otro número TIENE que
      // invalidar esta versión. Filtrando por canal, ese contador se congelaba y
      // los mensajes de la otra bandeja quedaban invisibles: el bug más
      // reincidente de este inbox, servido desde un lado nuevo.
      sb.from('bandeja').select('actualizado_en')
        .eq('cuenta', CUENTA)
        .order('actualizado_en', { ascending: false }).limit(1).maybeSingle(),
      sb.from('conversaciones').select('updated_at')
        .eq('cuenta', CUENTA)
        .order('updated_at', { ascending: false }).limit(1).maybeSingle(),
      // Los acuses (✓/✓✓/leído) cuentan AGRUPADOS en ventanas de 30 s: ver
      // acuseAgrupado. Sin filtrar por canal, por lo mismo que la de arriba.
      sb.from('bandeja').select('acuse_en')
        .eq('cuenta', CUENTA)
        .order('acuse_en', { ascending: false, nullsFirst: false }).limit(1).maybeSingle(),
    ])
    // Un error de cualquiera de las dos = no sabemos → se manda todo.
    //
    // ⚠️ Pero SE DICE por qué. La primera versión de esto se tragaba el error en
    // silencio, y cuando el ETag no cortaba nada no había forma de saber si era
    // porque el inbox estaba ocupado o porque la consulta fallaba. Un fallback
    // seguro que no deja rastro es el mismo pecado que venimos persiguiendo.
    if (b.error || c.error || a.error) {
      console.error('[versionInbox] bandeja:', b.error?.message || 'ok',
                    '| conversaciones:', c.error?.message || 'ok',
                    '| acuses:', a.error?.message || 'ok')
      return [null, null]
    }
    return [
      b.data?.actualizado_en || null,
      c.data?.updated_at || null,
      acuseAgrupado(a.data?.acuse_en, Date.now()),
    ]
  } catch (e) {
    console.error('[versionInbox] excepción:', e?.message)
    return [null, null]
  }
}
 ...
 */
// Sin tope: con 4000 la lista se llenaba hacia mediados de diciembre (2.518 hoy,
// +620/mes) y los chats más viejos desaparecían de GENERAL sin aviso.
export async function getListaSupabase(limite = Infinity, canal = canalPorDefecto()) {
  const sb = getSupabase()
  const filas = await paginarLimite(async (from, to) => {
    ...
    let q = sb
      ...
      .select('wa_message_id, telefono, nombre, tipo, texto, media_url, fecha, direccion, media_id, botones, estado_entrega, contexto_id, referral, phone_id, estado_bandeja, ultimo_entrante_canal, origen_anuncio')
      .eq('cuenta', CUENTA)
    if (canal) q = q.eq('phone_id', canal)   // canal null → todos los números (pestaña GENERAL)
    const { data, error } = await q
...
export async function guardarMensajeSupabase(m) {
  ...
  //
  // ☠️ Pero `ignoreDuplicates` significa GANA EL QUE LLEGA PRIMERO, y Meta manda
  // el MISMO wamid dos veces: primero un placeholder `unsupported` y ~0,4 s
  // después el mensaje de verdad, con su texto y su referral de pauta. Sin las
  // reparaciones de abajo, el placeholder se quedaba con el puesto: 135 mensajes
  // en 60 días quedaron como "no podemos mostrar" y 128 perdieron el anuncio del
  // que venía el cliente. El porqué y los números, en lib/reentrega.js.
  //
  // `.select()` distingue INSERTADA de YA EXISTÍA: con resolution=ignore-duplicates
  // PostgREST emite ON CONFLICT DO NOTHING ... RETURNING, que en un choque
  // devuelve CERO filas (verificado contra la base, no supuesto). Así el camino
  // caro corre solo en la reentrega y no en cada mensaje del webhook, que es la
  // ruta más caliente de las dos apps.
  const q = m.id
    ? sb.from('mensajes').upsert(fila, { onConflict: 'wa_message_id', ignoreDuplicates: true }).select('wa_message_id')
    : sb.from('mensajes').insert(fila)
  const { data, error } = await q
  if (error && !/duplicate key/i.test(error.message)) throw error

  // El wamid ya existía → lo guardado puede ser PEOR que lo que acaba de llegar.
  //
  // Si `data` no vino como se esperaba se asume que ya existía y se intenta
  // reparar igual: las guardias de cada reparación lo vuelven un no-op
  // inofensivo, y equivocarse hacia este lado cuesta una consulta — hacia el
  // otro cuesta el mensaje de un cliente.
  if (m.id && !(Array.isArray(data) && data.length > 0)) {
    for (const rep of reparacionesDeReentrega(fila)) {
      let r = sb.from('mensajes').update(rep.set).eq('cuenta', CUENTA).eq('wa_message_id', m.id)
      // La GUARDIA va en el WHERE, no en un `if` de JavaScript: los dos webhooks
      // llegan con 0,4 s de diferencia y pueden solaparse. Que decida Postgres.
      for (const [col, val] of Object.entries(rep.donde)) {
        r = val === null ? r.is(col, null) : r.eq(col, val)
      }
      const { error: errRep } = await r
      // Sin catch mudo a propósito: si esto fallara en silencio volveríamos al
      // bug de origen, que estuvo dos meses sin que nadie lo notara.
      if (errRep) throw errRep
    }
  }
  // HISTORIAL del celular (webhook `history` de la coexistencia): son mensajes
  // VIEJOS que llegan todos juntos al enganchar el número. Acá se corta a
  // propósito, ANTES de tocar la conversación o la bandeja:
  //   - `ultimo_entrante_at` se iría hacia ATRÁS (la ventana de 24 h mentiría), y
  //   - cada entrante viejo devolvería su chat a PENDIENTE: 200 chats de hace
  //     meses en la bandeja de un golpe, sin que nadie hubiera escrito hoy.
  // Un mensaje de historial se guarda para poder LEERLO; no significa que pasó algo.
  if (m.historial) return { ok: true }
  ...
  // ── EL CLIENTE SE MUDÓ DE TELÉFONO ─────────────────────────────────────────
  // Meta avisa por `system` que alguien cambió de número, y ahí su historial
  // queda PARTIDO: lo viejo bajo el teléfono anterior, lo nuevo bajo el otro, y
  // nadie los conecta. 38 casos, 31 en el último mes.
  //
  // El chat VIEJO ya dice a dónde se fue (etiqueta en wa-mensaje.js). Acá se
  // cierra la otra mitad: al número NUEVO se le deja un mensaje diciendo de
  // dónde viene esa persona.
  //
  // ⚠️ Entra como ENTRANTE, así que el chat cae en PENDIENTES. Eso NO es un
  // efecto secundario: es la alerta. Decisión de Rodrigo — prefiere una fila de
  // más en la bandeja antes que enterarse tarde de que un cliente se mudó.
  //
  // Recursivo a propósito: pasa por este mismo guardado para que la conversación
  // se cree y la bandeja se actualice igual que con cualquier mensaje. No hay
  // bucle: la nota lleva otro tipo en su `raw` y no vuelve a disparar otra
  // (fijado por prueba en tests/cambio-numero.test.js).
  const nota = notaParaNumeroNuevo(m)
  if (nota) await guardarMensajeSupabase(nota)
 ...
 *
 * ⚠️ Sigue siendo cierto que una persona pendiente en las dos bandejas aparece
 * en las dos: son dos conversaciones distintas y hay que contestarle por las dos.
 * Lo que NO puede pasar es contarla dos veces cuando solo debe una — y eso es
 * justo lo que le pasaba a IND hasta el 31-ago, porque su vista leía el estado
 * de `conversaciones` (UNO por persona) mientras la lista lee el de `bandeja`
 * (uno por persona Y canal). Acá nunca pasó: MANDI cuenta de `bandeja` desde el
 * principio. Ya están las dos iguales.
 *
...
export async function contarPendientesPorCanalSupabase() {
  ...
  //
  // ⚠️ Ya NO es cierto que la vista `pendientes_por_canal` tenga el modelo viejo:
  // el 31-ago se reescribió para leer de `bandeja`, o sea exactamente lo mismo
  // que este rpc (verificado: diferencia 0 en los 5 canales de las 2 cuentas).
  // Se sigue usando el rpc porque no hay razón para cambiar algo que anda; lo
  // que había que corregir era IND, no esto.
  const sb = getSupabase()
...
export async function getConversacionSupabase(telefono, limite = 40) {
  ...
  const t9 = soloDigitos(telefono).slice(-9)
  if (t9.length < 9) return []
  // ☠️ Antes traía TODOS los mensajes de la cuenta y filtraba el teléfono en
  // memoria: PostgREST corta en 1000, así que llegaban los 1000 MÁS VIEJOS de
  // toda la cuenta y cualquier cliente reciente quedaba con historial vacío.
  // Ahora se filtra en la base y se piden los más recientes.
  const { data, error } = await sb
    .from('mensajes').select('telefono, direccion, texto, fecha').eq('cuenta', CUENTA)
    .like('telefono', `%${t9}`)
    .order('fecha', { ascending: false }).limit(1000)
  if (error) throw error
  const delTel = (data || []).reverse().filter((m) => soloDigitos(m.telefono).slice(-9) === t9 && String(m.texto || '').trim())
  const turnos = []
...
// ── POSIBLES PAGOS SIN PEDIDO ──────────────────────────────────────────────
// Ver lib/pagos-sin-pedido.js para el porqué y los números. La consulta vive en
// la base (`inbox.pagos_sin_pedido`) porque cruza `inbox.mensajes` con
// `crm.pedidos` y eso no se arma con el cliente de Supabase.
export async function getPagosSinPedidoSupabase(desdeIso) {
  const sb = getSupabase()
  const { data, error } = await sb.rpc('pagos_sin_pedido', {
    p_cuenta: CUENTA,
    p_desde: desdeIso,
  })
  if (error) throw error
...
// Hasta cuándo se avisó. Sin esto el aviso repetiría los mismos casos cada día y
// se aprendería a ignorarlo — que es como muere un aviso.
export async function getMarcaAvisoPagosSupabase() {
  const sb = getSupabase()
  const { data } = await sb.from('app_config').select('valor')
    .eq('clave', `ultimo_aviso_pagos_${CUENTA}`).maybeSingle()
  return data?.valor || null
}

export async function setMarcaAvisoPagosSupabase(iso) {
  const sb = getSupabase()
  const { error } = await sb.from('app_config')
    .upsert({ clave: `ultimo_aviso_pagos_${CUENTA}`, valor: String(iso) }, { onConflict: 'clave' })
  if (error) throw error
  return { ok: true }
}

// ── FLUJOS (creador visual de nodos) ────────────────────────────────────────
// Ver docs/superpowers/specs/2026-09-15-flujos-lienzo-design.md. `grafo` es el
// borrador que se edita; `grafo_vivo` es la copia que corre (solo cambia al
// Publicar/Despublicar, nunca al Guardar).
export async function getFlujosSupabase() {
  const sb = getSupabase()
  const { data, error } = await sb.from('flujos')
    .select('flujo_id, nombre, publicado, grafo, grafo_vivo, creado_at, actualizado_at')
    .eq('cuenta', CUENTA).order('actualizado_at', { ascending: false })
  if (error) throw error
  return data || []
}

// Solo lo que el motor necesita para elegir y correr un flujo (el webhook no
// tiene por qué cargar el borrador de cada uno).
export async function getFlujosPublicadosSupabase() {
  const sb = getSupabase()
  // ☠️ `publicado` VA en el select aunque el WHERE ya filtre por él: tanto
  // `elegirFlujo` como `choquesDeDisparador` (lib/flujo.js) descartan la fila que
  // no traiga `f.publicado`. Sin la columna, esta consulta devuelve filas que el
  // motor tira TODAS y ningún flujo corre — en silencio, porque no hay error.
  const { data, error } = await sb.from('flujos')
    .select('flujo_id, nombre, publicado, grafo_vivo')
    .eq('cuenta', CUENTA).eq('publicado', true)
  if (error) throw error
  return data || []
}

// Upsert por flujo_id: con id -> update (la fila tiene que ser de esta cuenta);
// sin id -> insert. `publicado`/`grafo_vivo` nunca se tocan acá: eso es Publicar.
export async function guardarFlujoSupabase({ flujo_id, nombre, grafo }) {
  const sb = getSupabase()
  if (flujo_id) {
    const { data, error } = await sb.from('flujos')
      .update({ nombre, grafo })
      .eq('cuenta', CUENTA).eq('flujo_id', flujo_id)
      .select('flujo_id, nombre, publicado, grafo, grafo_vivo, creado_at, actualizado_at')
      .single()
    if (error) throw error
    return data
  ...
  const { data, error } = await sb.from('flujos')
    .insert({ cuenta: CUENTA, nombre, grafo })
    .select('flujo_id, nombre, publicado, grafo, grafo_vivo, creado_at, actualizado_at')
    .single()
  if (error) throw error
  return data
...
// publicado=true copia grafo -> grafo_vivo (lo que corre pasa a ser lo último
// guardado); false limpia grafo_vivo (despublicar deja el borrador intacto).
export async function setPublicadoFlujoSupabase(flujo_id, publicado) {
  const sb = getSupabase()
  if (publicado) {
    const { data: fila, error: errLectura } = await sb.from('flujos')
      .select('grafo').eq('cuenta', CUENTA).eq('flujo_id', flujo_id).single()
    if (errLectura) throw errLectura
    const { error } = await sb.from('flujos')
      .update({ publicado: true, grafo_vivo: fila.grafo })
      .eq('cuenta', CUENTA).eq('flujo_id', flujo_id)
    if (error) throw error
  } else {
    const { error } = await sb.from('flujos')
      .update({ publicado: false, grafo_vivo: null })
      .eq('cuenta', CUENTA).eq('flujo_id', flujo_id)
    if (error) throw error
  }
  return { ok: true }
...
export async function borrarFlujoSupabase(flujo_id) {
  const sb = getSupabase()
  const { error } = await sb.from('flujos').delete()
    .eq('cuenta', CUENTA).eq('flujo_id', flujo_id)
  if (error) throw error
  return { ok: true }
}
...
export async function setFlujoEstadoSupabase({ telefono, flujo_id, nodo_id, esperando, puerto_tiempo = null, vence_at, ultimo_wamid = '' }) {
  const sb = getSupabase()
  const { error } = await sb.from('flujo_estado').upsert({
    cuenta: CUENTA, telefono: telEstado(telefono), flujo_id, nodo_id, esperando,
    puerto_tiempo: puerto_tiempo || null, vence_at, ultimo_wamid: ultimo_wamid || null,
  }, { onConflict: 'cuenta,telefono' })
  if (error) throw error
  return { ok: true }
}
...
// día hay cientos, mejor varias pasadas de 5 min que una función que se muere.
