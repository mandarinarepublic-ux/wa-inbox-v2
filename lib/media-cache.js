...
export function esErrorDeMediaId(data) {
  ...
  if (msg.includes('media') && (msg.includes('not exist') || msg.includes('invalid') || msg.includes('expired') || msg.includes('unsupported'))) return true
  // Un media_id subido por OTRO número (la caché es por phone_id) o ya borrado no da
  // un error "de media": da el genérico (#100) "Object with ID '...' does not exist,
  // cannot be loaded due to missing permissions...". Para nosotros significa lo
  // mismo —ese id no sirve acá—: hay que re-subir la foto por este canal.
