...
export async function enviarEventoCapi({ evento, telefono, value = null }) {
  try {
    ...
    if (!res.ok) {
      ...
      const esCulpaNuestra = [190, 102, 10, 200, 294].includes(codigo)

      if (res.status >= 500 || esCulpaNuestra) {
        await sb.from('capi_events').delete().eq('event_id', eventId)
        return { error: detalle, codigo, reintentable: true }
      }
      return { error: detalle, codigo, status: res.status }
    }
