...
export async function revisarVentaEnProceso(telefono) {
  try {
    ...
    const { data: conv } = await sb
      .from('conversaciones')
      .select('ctwa_clid, ctwa_captured_at, temperatura, estado, etapa, tipo_contacto')
      .eq('cuenta', CUENTA)
      .eq('telefono', telefono)
      .maybeSingle()
    ...
    // 💳 Esperando pago / 🛒 Falta pedido (port desde IND) se SUMAN a 🔥/SOPORTE.
    // El 🔥 manual se retira en la etapa 2 del port, junto con la pantalla nueva.
    let motivo = caliente ? 'caliente' : soporte ? 'soporte' : motivoPorEtapa(conv.etapa)
