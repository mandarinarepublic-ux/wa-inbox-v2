...
import { wabaIdDePhoneId, LEAD_UMBRAL_DEFECTO } from './canales.js'
 ...
 *
 * El valor por marca vive en lib/canales.js, que es donde ya está lo específico
 * de cada inbox — este archivo es IDÉNTICO en los dos a propósito. Hoy: IND 4
 * (genera 65 señales/semana, de sobra) y MANDARINA 3 (con 4 se quedaba en 33 y
 * Meta necesita ~50 para aprender).
 */
export const LEAD_UMBRAL = Number(process.env.CAPI_LEAD_UMBRAL || LEAD_UMBRAL_DEFECTO)
...
/**
 * El token que publica es META_TOKEN, el de la Cloud API — NO uno de CAPI.
 *
 * Un token de CAPI se genera desde un dataset concreto y solo sirve para ese: con
 * los datasets de mensajería devuelve "Object with ID … cannot be loaded due to
 * missing permissions" (subcode 33). META_TOKEN en cambio manda sobre la WABA, y
 * es el que CREA estos datasets, así que también puede publicar en ellos.
 * Verificado con la sonda de /api/capi/diag el 1-ago-2026.
 *
 * O sea que no hay que generar ni mantener ningún token nuevo para esto.
 */
export function capiConfigurado() {
  return Boolean(env('META_TOKEN'))
}

/**
 * Dataset al que van los eventos de ESTA WABA.
 *
 * Los eventos de Click-to-WhatsApp NO van al pixel del sitio web. Meta exige un
 * dataset atado a la cuenta de WhatsApp Business, y no hay forma de asociarlos
 * desde el panel — esa sección simplemente no existe en un dataset web (se
 * recorrió entera el 1-ago buscándola). La asociación se hace creándolo por API,
 * y entonces queda atado solo:
 *
 *   POST /{WABA_ID}/dataset  →  crea el dataset, o devuelve el que ya exista
 *
 * Se resuelve una vez por WABA y se cachea en inbox.capi_datasets. Va en la base
 * y no en una variable de entorno a propósito: acá los números se migran de WABA
 * seguido, y así uno nuevo se resuelve solo la primera vez que alguien escribe,
 * sin desplegar nada.
 *
 * Usa META_TOKEN (el de la Cloud API, que tiene permiso sobre la WABA), NO
 * META_CAPI_TOKEN.
 */
async function datasetDeWaba(sb, wabaId) {
  const { data: fila } = await sb
    .from('capi_datasets').select('dataset_id').eq('waba_id', wabaId).maybeSingle()
  if (fila?.dataset_id) return fila.dataset_id

  const token = env('META_TOKEN')
  if (!token) return null

  const r = await fetch(`${GRAPH}/${wabaId}/dataset`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ access_token: token }),
  })
  const b = await r.json().catch(() => ({}))
  const id = b?.id || b?.dataset_id
  if (!r.ok || !id) {
    console.error(`[capi] dataset de la WABA ${wabaId}:`, b?.error?.error_user_msg || b?.error?.message || `HTTP ${r.status}`)
    return null
  }

  // onConflict: dos mensajes del mismo lote pueden resolverlo a la vez.
  await sb.from('capi_datasets').upsert({ waba_id: wabaId, dataset_id: String(id) }, { onConflict: 'waba_id' })
  return String(id)
}
...
export async function enviarEventoCapi({ evento, telefono, value = null }) {
  try {
    ...
    // El dataset depende de la WABA por la que entró el click, igual que el
    // whatsapp_business_account_id. Una variable de entorno única no alcanza.
    const datasetId = await datasetDeWaba(sb, wabaId)
    if (!datasetId) return { omitido: 'sin_dataset_para_la_waba', wabaId }
    ...
    const { error: errInsert } = await sb.from('capi_events').insert(marca)

    if (errInsert) {
      // Puede que la marca esté TRABADA en vez de enviada.
      //
      // La fila se inserta antes de llamar a Meta, así que si la función se muere
      // en el medio (Vercel la corta, se cae la red de forma dura) queda con
      // http_status en null: nunca se envió, pero el UNIQUE ya le cerró la puerta
      // a ese contacto para siempre. Pasó 1 vez en 14 el 1-ago, y en silencio.
      //
      // Se destraba acá y no en un cron a propósito: este es el momento exacto en
      // que hace falta —el cliente acaba de escribir— y no cuesta nada, porque
      // solo corre cuando el insert rebota. Un cron revisaría miles de filas sanas
      // para encontrar una rota.
      const { data: previa } = await sb
        .from('capi_events')
        .select('id, http_status, sent_at')
        .eq('event_id', eventId)
        .maybeSingle()

      // 10 min: una función serverless no vive ni de lejos tanto, así que si
      // sigue sin resolver es que murió, no que esté en vuelo.
      const trabada = previa
        && previa.http_status === null
        && (Date.now() - new Date(previa.sent_at).getTime()) > 10 * 60_000

      if (!trabada) return { omitido: 'ya_enviado' }

      await sb.from('capi_events').delete().eq('id', previa.id)
      const { error: errReintento } = await sb.from('capi_events').insert(marca)
      // Si otro proceso ganó la carrera, que se quede él con el envío.
      if (errReintento) return { omitido: 'ya_enviado' }
      console.warn(`[capi] ${evento} ${telefono}: marca trabada liberada, reintentando`)
    }
    ...
    let res, body
    try {
      res = await fetch(`${GRAPH}/${datasetId}/events`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
/** 💳 Esperando pago o 🛒 Falta pedido cuentan como venta en proceso (port desde IND). */
export function motivoPorEtapa(etapa) {
  return etapa === 'esperando_pago' || etapa === 'falta_pedido' ? etapa : null
}

/**
 * Venta en proceso → InitiateCheckout.
 *
 * Definición del negocio (1-ago-2026, ampliada el 8-ago): el chat cuenta como
 * venta en proceso si se cumple CUALQUIERA de las cuatro:
 *
 *   - está marcado 🔥 CALIENTE
 *   - está en la bandeja SOPORTE (desde el 8-ago se marca A MANO: el webhook ya
 *     no manda chats ahí, porque todos tienen que caer en Pendientes)
 *   - el cliente MANDÓ UNA FOTO (agregada el 8-ago para no perder la señal que
 *     antes llegaba de rebote por la bandeja SOPORTE — ver más abajo)
...
export async function revisarVentaEnProceso(telefono) {
  try {
    ...
    const { data: conv } = await sb
      .from('conversaciones')
      .select('ctwa_clid, ctwa_captured_at, etapa, tipo_contacto')
      .eq('cuenta', CUENTA)
      .eq('telefono', telefono)
      .maybeSingle()
    ...
    // Contacto interno (equipo, taller): no es una venta que Meta deba aprender.
    if (conv.tipo_contacto === 'interno') return { omitido: 'contacto_interno' }

    // La marca humana es la etapa 💳 Esperando pago / 🛒 Falta pedido (port desde
    // IND). 🔥 CALIENTE y SOPORTE se retiraron el 23-sep: la temperatura ya es
    // automática (no es una decisión de nadie) y SOPORTE dejó de ser bandeja.
    let motivo = motivoPorEtapa(conv.etapa)

    // ¿Mandó una foto? Es la señal más temprana y más fiable de que va a comprar:
    // casi siempre es la prenda que quiere, o una captura del catálogo.
    //
    // Antes esto llegaba de rebote: el webhook ponía esos chats en la bandeja
    // SOPORTE y la línea de arriba lo leía. El 8-ago se quitó ese automatismo —los
    // chats tienen que caer SIEMPRE en Pendientes— así que la señal se pide ahora
    // por el hecho real en vez de por la bandeja. Queda mejor que antes: ya no
    // depende de un estado que una persona puede cambiar sin querer.
    //
    // ⚠️ Se miran DOS nombres de tipo: 'imagen' es el de hoy e 'image' el de los
    // mensajes anteriores al 11-jul-2026 (de antes de la migración). Mirando uno
    // solo, la señal no saltaría para media base y nadie se enteraría.
    if (!motivo) {
      const { count: fotos } = await sb
        .from('mensajes')
        .select('mensaje_id', { count: 'exact', head: true })
        .eq('cuenta', CUENTA)
        .eq('telefono', telefono)
        .eq('direccion', 'ENTRANTE')
        .in('tipo', ['imagen', 'image'])
      if ((fotos || 0) > 0) motivo = 'foto'
    }
