...
export function nodoDisparador(grafo) {
  return (Array.isArray(grafo?.nodos) ? grafo.nodos : []).find((n) => n?.tipo === 'disparador') || null
}
 ...
 *    cliente escribió en vez de tocar). Los botones mandan sobre esperarRespuesta:
...
const idNodo = (grafo, id) => (Array.isArray(grafo?.nodos) ? grafo.nodos : []).find((n) => n.id === id)
...
export function validarFlujo(grafo, { respuestas = [] } = {}) {
  ...
  const lineas = Array.isArray(grafo?.lineas) ? grafo.lineas : []
  ...
  if (disparadores.length === 1) {
    ...
    if (dt.tipo === 'palabra' && !(Array.isArray(dt.palabras) && dt.palabras.length)) {
  ...
  for (const l of lineas) {
    ...
    if (espera < 0 || espera > MAX_ESPERA_MIN) {
  ...
  for (const n of nodos) {
    ...
    if (botonesRaw.length > MAX_BOTONES || botonesRaw.some((b) => tituloDe(b).length > MAX_TITULO)) {
      errores.push({ nodoId: n.id, texto: `máximo ${MAX_BOTONES} botones de ${MAX_TITULO} letras` })
    }
  ...
  // Ciclos: solo cuentan las líneas SIN espera (esperaMin 0) — un ciclo con espera
  // el motor lo corta solo porque cada vuelta consume la ventana de 24h.
  const adyacencia = new Map()
  for (const l of lineas) {
    if ((Number(l.esperaMin) || 0) > 0) continue
...
function motivoDeChoque(propio, otro) {
  ...
  if (propio?.tipo === 'palabra' && otro?.tipo === 'palabra') {
    const propias = new Set((propio.palabras || []).map(normalizarTexto))
    if ((otro.palabras || []).some((p) => propias.has(normalizarTexto(p)))) return 'misma palabra'
  }
  return null
}
...
/**
 * A qué flujo publicado le toca este entrante. Prioridad: anuncio (sourceId) >
 * palabra (contenida en el texto) > orgánico (solo si es contacto nuevo). Solo
 * mira flujos `publicado && grafo_vivo`: un borrador nunca corre.
 */
export function elegirFlujo({ flujos, sourceId, esNuevo, texto }) {
  ...
  if (sid) {
    ...
    if (porAnuncio) return porAnuncio
  }
  const norm = normalizarTexto(texto)
  if (norm) {
    const porPalabra = candidatos.find((f) => {
      const d = nodoDisparador(f.grafo_vivo)
      return d?.datos?.tipo === 'palabra' && (d.datos.palabras || []).some((p) => norm.includes(normalizarTexto(p)))
    })
 ...
 * "siguiente", acumulando los nodos Mensaje de paso. Se detiene, en este orden:
 *  1. la línea que toca seguir tiene espera (`esperaMin>0`) → 'espera' (el nodo
 *     destino NO se agrega: todavía no llegó).
 *  2. el nodo destino es Condición → 'condicion' (Fase B decide el sí/no).
 *  3. el nodo destino es Mensaje → SIEMPRE se agrega; si tiene botones → 'botones'
 *     (Fase B espera el toque); si `esperarRespuesta` → 'esperar_respuesta'.
 *  4. el nodo destino es Fin, o el puerto no tiene línea → 'fin'.
 *  5. el nodo destino no existe (línea rota) → 'huerfano'.
 */
export function caminoLineal(grafo) {
  ...
  for (let paso = 0; paso < TOPE_PASOS_CAMINO; paso++) {
    ...
    if ((Number(linea.esperaMin) || 0) > 0) return { mensajes, detenidoEn: actualId, motivo: 'espera' }
    ...
    if (destino.tipo === 'fin') return { mensajes, detenidoEn: actualId, motivo: 'fin' }
...
export function piezasDeNodos({ nodos, respuestas, contacto, citaId = '' }) {
  ...
  for (const nodo of Array.isArray(nodos) ? nodos : []) {
    const dt = nodo?.datos || {}
    let texto
    let botones
    let adjuntos
    ...
    if (dt.origen === 'respuesta') {
      ...
      adjuntos = Array.isArray(dt.adjuntos) ? dt.adjuntos : []
    }
...
export function temperaturaAlPasar(nodos) {
  let ultima = ''
  for (const n of Array.isArray(nodos) ? nodos : []) {
