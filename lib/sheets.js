...
// ── Leer una hoja de OTRO spreadsheet (misma Service Account) ─────
// Para el dashboard combinado, que lee la hoja de MANDI y la de IND.
export async function readSheetFrom(spreadsheetId, sheetName) {
...
// Caché en memoria del conteo de filas por hoja. readSheetTail hacía DOS llamadas
// a la API por request (metadata + valores) — duplicando el gasto en el endpoint
// más caliente. El total de filas crece lento, así que lo cacheamos 60s y evitamos
// la llamada de metadata en la mayoría de los polls.
const rowCountCache = new Map() // sheetName → { count, at }
const ROWCOUNT_TTL = 60_000

async function getRowCount(sheets, sheetName) {
  const hit = rowCountCache.get(sheetName)
  if (hit && Date.now() - hit.at < ROWCOUNT_TTL) return hit.count
  ...
  const count = sheet?.properties?.gridProperties?.rowCount || 15000
  rowCountCache.set(sheetName, { count, at: Date.now() })
  return count
}
...
export async function readSheetTail(sheetName, lastN = 3000) {
  const sheets = await getSheets()
  const totalRows = await getRowCount(sheets, sheetName)
