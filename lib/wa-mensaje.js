...
export function extraer(msg) {
  ...
  switch (msg.type) {
    ...
    case 'order':    return base({ tipo: 'order', contenido: formatearPedido(msg.order), mediaId: '' })
    ...
    case 'unsupported': {
      const titulo = msg.errors?.[0]?.title ? ` (${msg.errors[0].title})` : ''
      return base({ tipo: 'unsupported', contenido: `⚠️ Te escribió algo que no podemos mostrar${titulo}`, mediaId: '' })
    }
    ...
    case 'system':   return base({ tipo: 'system', contenido: 'ℹ️ Aviso de WhatsApp', mediaId: '' })
